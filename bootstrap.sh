...
# Just an example for users in case we forget to update the chart version in our docs
echo "e.g: helm upgrade <YOUR_CLUSTER_NAME> capi/openstack-cluster --install -f values.yaml -f clouds.yaml -f user-values.yaml -f flavors.yaml --version ${CLUSTER_CHART} --wait -n clusters"
